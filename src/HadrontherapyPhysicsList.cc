...
HadrontherapyPhysicsList::HadrontherapyPhysicsList() : G4VModularPhysicsList()
{
    ...
    AddPhysicsList("HADRONTHERAPY_2");
...
void HadrontherapyPhysicsList::ConstructProcess()
{
    ...
    //Parallel world sensitivity (for scorers overlapping with the mass world: not used!)
    //
...
void HadrontherapyPhysicsList::AddPhysicsList(const G4String& name)
{
    ...
    if (name == "standard_opt4") {
        ...
        //     the ion-ion inelastic interactions for generic ions
	//     But it uses BinaryLightIonReaction with Tripathi cross sections for helium!
...
void HadrontherapyPhysicsList::SetCuts()
{
    ...
    G4Region* region_scint = G4RegionStore::GetInstance()->GetRegion("energy_calorimeter");
    G4ProductionCuts* cuts_scint = new G4ProductionCuts;
    cuts_scint->SetProductionCut(50.0*um);
    region_scint->SetProductionCuts(cuts_scint);
}
